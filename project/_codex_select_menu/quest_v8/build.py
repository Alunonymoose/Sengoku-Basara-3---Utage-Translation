...
O=W.parent/'quest_v8';OUT=O/'release';ST=OUT/'root';ST.mkdir(parents=True,exist_ok=True)
def rows(raw):
...
for i,text in TRANSLATIONS.items():
 ...
 assert controls==([65426,65425] if i==489 else []),(i,'Unexpected controls')
 vals=[]
 for ch in text:
  if ch=='\n':
   vals.append(65534)
   if i==489:vals.extend([65426,2])
   continue
  assert ch in mp and mp[ch]!=65535,(i,ch)
  vals.append(mp[ch])
 rr[i]=vals+([65425] if i==489 else [])+[65535];widths[i]=max(sum(struct.unpack_from('>H',tnf,32+mp[ch]*8+6)[0] for ch in line) for line in text.split('\n'))
 assert widths[i]<=(190 if i<400 else 720),(i,widths[i])
new=rebuild(original,rr);assert rows(new)==rr
files=[]
with zipfile.ZipFile(OUT/'BACKUP_PRE_V8.zip','w',zipfile.ZIP_DEFLATED) as z:
