...
O=W.parent/'quest_v8';OUT=O/'release';r=json.loads((OUT/'VALIDATION.json').read_text());texts={int(i):s for i,s in json.loads((O/'TRANSLATIONS.json').read_text()).items()}
def rows(raw):
...
def decode_text(v):
 ...
 return s
with zipfile.ZipFile(OUT/'BACKUP_PRE_V8.zip') as z:
 ...
 for rec in r['files']:
  ...
  for e,f in zip(a.entries,b.entries):
   ...
   assert [v for v in new[489] if v>=0xf000]==[65534,65426,65425,65535]
...
result=dict(status='pass',translated_items=40,translated_records=80,archives=r['archives'],changed_resources=r['changed_resources'],untouched_resources=r['untouched_resources'],checks=['Backup CRC and hashes','All 80 English records decoded and compared','Untouched compressed entries identical','Record count and controls preserved','Glyph coverage and width limits','Native font proof inspected'],runtime_verified=False)
