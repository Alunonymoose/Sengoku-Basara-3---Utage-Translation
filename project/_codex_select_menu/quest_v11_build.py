...
sys.path.insert(0,r'E:\\Utage Patching New\\_codex_select_menu\\quest_v10');from translations import TRANSLATIONS

O=W.parent/'quest_v11';OUT=O/'release';ST=OUT/'root';ST.mkdir(parents=True,exist_ok=True)
...
for idx,text in list(TRANSLATIONS.items()):
 plain=text.replace('<c>','').replace('</c>','')
 if any(ch not in mp or mp[ch]==65535 for ch in plain): continue
 if idx>=400:
  wrapped=[]
...
':vals.append(65534);continue
   if ch not in mp or mp[ch]==65535: vals=[]; break
   vals.append(mp[ch])
...
files=[]
with zipfile.ZipFile(OUT/'BACKUP_PRE_V11.zip','w',zipfile.ZIP_DEFLATED) as z:
