...
r.update(status='offline_validated',backup=str(OUT/'BACKUP_PRE_V7.zip'),backup_sha256=sha((OUT/'BACKUP_PRE_V7.zip').read_bytes()));(OUT/'VALIDATION.json').write_text(json.dumps(r,indent=2))
package=OUT/'Utage_Menu_V7_MAP_NAMES_AND_SHOP_ROOT_READY.zip'
...
try:
 for rec in r['files']:
  target=ROOT/rec['path'];temp=target.with_name(target.name+'.menu_v7_pending');assert not temp.exists();temp.write_bytes((OUT/'root'/rec['path']).read_bytes());assert sha(temp.read_bytes())==rec['sha256'];pending.append((rec,target,temp))
 ...
 with zipfile.ZipFile(OUT/'BACKUP_PRE_V7.zip') as bk:
