...
static byte[] BuildSyntheticArc(bool corruptSecondOffset = false)
{
    const int entrySize = 80;
    const int tableStart = 8;
    const int payload1Offset = 0x800;
    const int payload2Offset = 0x810;
...
using (var stream = new MemoryStream(bytes, writable: false))
{
    ...
    Smoke.Equal("TEST_TEXTURE_0001", Encoding.ASCII.GetString(UtageArcReader.ReadDecompressedPayload(stream, arc.Entries[0])), "stored payload round-trip");
}
...
Smoke.Throws<NotSupportedException>(() =>
{
    ...
    little[0] = (byte)'A'; little[1] = (byte)'R'; little[2] = (byte)'C'; little[3] = 0;
